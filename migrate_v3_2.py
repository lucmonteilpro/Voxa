"""
Voxa — Migration v3.2 (Quality Controller v2 — colonnes qc_v2_*)
================================================================
Ajoute les 7 colonnes nécessaires au QC v2 sur la table `action_items`
dans `voxa_accounts.db` UNIQUEMENT.
...
- qc_v2_status              TEXT
- qc_v2_score_baseline      INTEGER
- qc_v2_score_test_median   INTEGER
- qc_v2_delta               INTEGER
- qc_v2_verdicts_json       TEXT
- qc_v2_run_id              INTEGER
- qc_v2_validated_at        TEXT

Patterns vs migrate_v3.py :
- BACKUP : repris (shutil.copy2 + timestamp), format ISO-safe macOS
  (`%Y-%m-%dT%H-%M-%S`, pas de `:` interdit sur macOS).
- IDEMPOTENCE : NOUVEAU pattern PRAGMA table_info + check par colonne.
  SQLite ne supporte pas ALTER TABLE … ADD COLUMN IF NOT EXISTS.
- SMOKE TEST post-migration : NOUVEAU. Vérifie 3 invariants :
    1. n_colonnes_avant + n_added == n_colonnes_après
    2. COUNT(action_items) inchangé
    3. valeurs legacy (status, score_real, measured_at) inchangées
       sur les lignes existantes (snapshot avant/après).
- RESTORE : repris (--restore depuis backup le plus récent).

En cas d'échec à n'importe quelle étape : restore depuis backup + exit 1.

Usage :
    python3 migrate_v3_2.py            # migration normale
    python3 migrate_v3_2.py --dry-run  # preview, ne touche rien
    python3 migrate_v3_2.py --restore  # restore depuis dernier backup v3_2
"""
...
# Colonnes legacy v1 à préserver (vérification post-migration)
LEGACY_COLUMNS_TO_PRESERVE = ("status", "score_real", "measured_at")
...
def _existing_columns(conn: sqlite3.Connection, table: str) -> set:
    """Retourne l'ensemble des noms de colonnes via PRAGMA table_info."""
...
def _snapshot_legacy_rows(conn: sqlite3.Connection) -> list:
    """Snapshot (id, status, score_real, measured_at) pour comparaison."""
...
def _backup_path() -> Path:
    """Format ISO-safe macOS : YYYY-MM-DDTHH-MM-SS (`-` au lieu de `:`)."""
    ts = datetime.now().strftime("%Y-%m-%dT%H-%M-%S")
    return ACCOUNTS_DB.parent / f"{ACCOUNTS_DB.stem}.backup_v3_2_{ts}.db"


def _list_backups_v3_2() -> list:
    pattern = f"{ACCOUNTS_DB.stem}.backup_v3_2_*.db"
    return sorted(ACCOUNTS_DB.parent.glob(pattern), reverse=True)


def _restore_latest() -> Path:
    backups = _list_backups_v3_2()
    if not backups:
        raise FileNotFoundError(f"Aucun backup v3_2 trouvé pour {ACCOUNTS_DB}")
    latest = backups[0]
...
# ─────────────────────────────────────────────
# Skip log (audit trail des autres DBs)
# ─────────────────────────────────────────────
def _log_skips() -> None:
    """Affiche le SKIP des autres DBs voxa*.db (audit trail P3)."""
    other_dbs = sorted(
...
def _diagnose() -> dict:
    ...
    try:
        ...
        missing = [(name, sql_type) for name, sql_type in QC_V2_COLUMNS
...
def _migrate(dry_run: bool = False) -> dict:
    ...
    conn = sqlite3.connect(ACCOUNTS_DB)
    try:
        # Snapshots avant ALTER pour smoke test
        ...
        # Smoke test post-migration (P2 : 3 invariants)
        ...
        if snapshot_after != snapshot_before:
            raise RuntimeError(
                "smoke fail (legacy) : valeurs status/score_real/measured_at "
        ...
        raise RuntimeError(
            f"Migration v3.2 FAILED, restored from {backup.name}. Error: {e}"
        )
    finally:
        try:
            conn.close()
...
def main():
    parser = argparse.ArgumentParser(
        description="Voxa Migration v3.2 — colonnes qc_v2_* sur action_items",
    ...
    parser.add_argument("--restore", action="store_true",
                        help="Restaure voxa_accounts.db depuis le dernier backup v3_2")
    ...
    print(f"{mode}Migration v3.2 (qc_v2_* sur action_items)\n")

    # 1) Audit trail des autres DBs (P3)
    _log_skips()
    print()

    # 2) Migration ciblée voxa_accounts.db
    try:
    ...
    if stats["skipped"]:
        print(
            ...
            f"(toutes les colonnes qc_v2_* sont présentes, "
